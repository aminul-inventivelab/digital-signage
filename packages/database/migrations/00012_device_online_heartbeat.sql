-- The portal reads devices.status and last_seen; they were only set to offline at link time
-- and never updated while the TV was running. Mark online + heartbeat from real TV traffic.

-- Mark online whenever the screen successfully asks for the manifest (poll every few seconds).
...
-- Redundant, slower heartbeat from telemetry; keeps online between playlist polls.
