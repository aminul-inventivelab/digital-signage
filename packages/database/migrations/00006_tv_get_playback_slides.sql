-- TV loads playback in one call (bypasses fragile multi-table PostgREST + RLS for anon).
-- Enforces: caller must be the device row's registering session (auth.uid() = registered_session_id).
