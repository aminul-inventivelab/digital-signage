-- Replaces the TV playback RPC to expose slides without in-file trim (use duration_seconds for max on-screen time only).
-- The previous version of this migration added clip_start_ms / clip_end_ms; 00009 drops those columns for databases that applied it.
