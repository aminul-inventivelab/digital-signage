-- Efficient manifest sync: cheap revision RPC, heartbeat separated from manifest fetch
-- (avoids Realtime feedback loops when the TV subscribes to `devices` / `device_playlists`).

-- ---------------------------------------------------------------------------
-- Lightweight manifest fingerprint (same auth + hash as tv_get_playback_slides; no slide aggregation).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Liveness only — call on an interval from the TV (does not aggregate slides).
-- ---------------------------------------------------------------------------
create or replace function public.tv_device_heartbeat(p_device_id uuid, p_playback_secret text default null)
returns void
language plpgsql
volatile
security definer
...
revoke all on function public.tv_device_heartbeat(uuid, text) from public;
grant execute on function public.tv_device_heartbeat(uuid, text) to anon, authenticated;

-- ---------------------------------------------------------------------------
-- Manifest fetch: remove last_seen/status side-effect (use tv_device_heartbeat instead).
-- ---------------------------------------------------------------------------
